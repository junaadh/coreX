...
use core_x::frontend::{build_hir_local_binding_table, ExpansionOptions, expand_parsed_files, desugar_files, lower_to_hir};

fn parse_and_lower(source: &str) -> (core_x::frontend::hir::HirFile, core_x::frontend::hir::HirModule) {
...
fn test_foo_x_int_and_foo_y_int_have_same_signature() {
    ...
    let sig1 = if let core_x::frontend::hir::HirItemKind::Function(f) = &func1_item.kind {
    ...
    let sig2 = if let core_x::frontend::hir::HirItemKind::Function(f) = &func2_item.kind {
        CallSignature::from_hir_function(f)
    } else {
        panic!("expected function")
    };
    ...
    // Both have FromName, so signatures are equal
    assert_eq!(sig1, sig2, "FromName(x) and FromName(y) have same signature");
...
fn test_foo_underscore_x_int_and_foo_x_int_have_different_signatures() {
    ...
    assert_ne!(sig1, sig2, "None(x) and FromName(x) have different signatures");
    assert_eq!(sig1.param_count(), 1);
    assert_eq!(sig2.param_count(), 1);

    // First has None (no label)
    assert_eq!(sig1.params[0].label, core_x::frontend::resolver::CallParamLabel::None);
    assert!(!sig1.accepts_label_at(0));

    // Second has FromName (label from argument)
    assert_eq!(sig2.params[0].label, core_x::frontend::resolver::CallParamLabel::FromName);
...
fn test_foo_label_x_int_has_different_signature() {
    ...
    let sig = if let core_x::frontend::hir::HirItemKind::Function(f) = &func_item.kind {
        CallSignature::from_hir_function(f)
    } else {
        panic!("expected function")
    };
    ...
    assert!(sig.accepts_label_at(0), "explicit label requires label at call site");
...
fn test_multiple_params_with_different_labels() {
    ...
    assert_eq!(sig1.external_labels(), vec![None, None, Some("label".to_string())]);
...
fn test_init_functions_have_signature_with_origin() {
    ...
    // Get the struct
    let struct_item = hir_module.items.get(&hir_file.root_items[0]).expect("item should exist");
    let struct_decl = if let core_x::frontend::hir::HirItemKind::Struct(s) = &struct_item.kind {
        s
    } else {
